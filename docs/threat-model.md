...
An attacker may: submit arbitrary HTTP requests to `services/api`; hold a validly-generated keypair of their own; observe network traffic (assume TLS is terminated correctly outside this application's scope); attempt to replay, reorder, or partially replay previously-observed requests; possess a compromised actor key; control a federation peer.
